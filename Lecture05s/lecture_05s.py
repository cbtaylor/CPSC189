...
def average_file(f):
    ...
    Requires: at most one real number per line:
              there is at least one real number in the file
...
def average_file_no_header(f, delim=None):
    ...
    Requires: reals are separated by the delim string,
...
def average_file_header(f, delim=None, num_header_rows=0):
    ...
    Produces the average of data in file f
    Requires: data is separated by delim, there are num_header_rows at top
              of file to be ignored

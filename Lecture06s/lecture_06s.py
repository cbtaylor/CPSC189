...
def average_file_dir(rdir, delim=None, num_header_rows=0):
    ...
    Effect: computes the average of data stored in each .csv file found
            in the directory structure rooted at rdir and saves result in
            a .txt file in same directory as .csv file.
    """
    for root, lodn, lofn in os.walk(rdir):
...
def average_file_lof(root, lofn, delim=None, num_header_rows=0):
    ...
    Effect: computes the average of data stored in each .csv file found
            in lofn and saves result in .txt file in same directory as
            .csv file.
    """
    for fn in lofn:
...
def plot_posn_theoretical(lot):
    ...
    Effect: generates plot of position against time for times in lot
    """
...
def plot_posn_empirical(f):
    """
    str, str -> NoneType
    
    Effect: generates a plot of position against time for data read from
            file f.
    Requires: each line of f contains a time and a position, no headers
    """
    with open(f, 'U') as inf:
        ...
        for line in inf:
            ...
            posns.append(float(p))
        
        pyplot.plot(times, posns, 'r^', linestyle='-', linewidth=1)
